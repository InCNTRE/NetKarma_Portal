package NetKarma::XGMML;

=pod

=head1 NAME

NetKarma::XGMML - Simple Graph.pm-like interface for generating XGMML graphs
...
sub add_node {
	...
	foreach my $att (@_) {
		...
		my $val = $att;
		print "ATT: $nm :" . _get_type($val) . " : $val\n";
		
	        $self->{writer}->startTag('att',name => $nm, type => _get_type($val), value => $val);
                $self->{writer}->endTag('att');;
            }
	    $i++;
	} 
...
sub _get_type {
	...
	# Return Java type of a scalar
	if (($scalar eq "true") ||  ($scalar eq "false")) {
	   $type = "boolean";
	} 
	elsif ($scalar =~ m/[^-+.\d]/ || $scalar !~ /\d/) { 
	   $type= "string" }
        elsif ($scalar == int($scalar)) { 
           $type = "integer" }
        else { $type = "float" }

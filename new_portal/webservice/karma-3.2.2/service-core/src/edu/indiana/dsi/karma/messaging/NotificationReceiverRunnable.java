...
public class NotificationReceiverRunnable  implements Runnable  {
	...
	public void run() throws java.lang.IllegalMonitorStateException{
		...
		while (runInfinite) {
			try {
				log.info("[Karma server: Listening to Messaging System]");
				nofitication=this.receiver.getMessage();
				log.info("[Karma server: One Message received]\n"+nofitication);
				
				KarmaOperations.storeNotification(this.ingester, 
												NotificationTypeEnum.determineNotificationTypeFromXmlBeansDocument(XmlObject.Factory.parse(nofitication)),
												nofitication);

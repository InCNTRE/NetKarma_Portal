...
public class QueryReceiverRunnable  implements Runnable  {
	...
	public void run() throws java.lang.IllegalMonitorStateException{
		...
		while (runInfinite) {
			try {
				log.info("[Karma server: Listening Queries from Messaging System]");
				queryMessage=this.receiver.getMessage();
				log.info("[Karma server: One Query received]\n"+queryMessage);
				...
				if(queryMessage!=null){
					...
					if (queryMessageContents.length!=2) {
						...
						try{
							queryResult=KarmaOperations.queryProvenance(querier, XmlObject.Factory.parse(query));
						}catch(Exception e){
							
							queryResult=INVALID_QUERY_ERROR_STRING;
							log.info("[Karma server: Query Error] "+e.toString());
						}
						//Response
						...
						try {
							...
							log.info("[Karma server: Send Query Response to Client]");
							sender.closeChannel();
							sender.closeConnection();
						} catch (ShutdownSignalException e) {
							// TODO Resend to client
							log.info("[Karma server: Query Messaging Response Error] "+e.toString());
						} catch (IOException e) {
							// TODO Resend to client
							log.info("[Karma server: Query Messaging Response Error] "+e.toString());
					...
					log.info("[Karma server: Empty Query Request]\n");
				}

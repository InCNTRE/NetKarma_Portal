...
-- DROP TABLE IF EXISTS reg_agent_entity;
-- CREATE TABLE reg_agent_e               BIGINT NOT NULL,                           -- foreign key to reg_entity.entity_internal_id
--  agent_name                           VARCHAR(511) NOT NULL,                     -- a name of the agent, e.g. URI
--  version                ntity (
--  agent_internal_id                   VARCHAR(31) NULL,                          -- optional version information for the agent/workflow
--  creation_time                        DATETIME NOT NULL,                         -- time the agent was created 
--  
--  -- QUESTION: is agent collection an execution level notion, or a registry level notion?
--  cardinality                          VARCHAR(15) NOT NULL,                      -- STANDALONE or COLLECTION
--  collection_internal_id               BIGINT NULL,                               -- foreign key to reg_agent_entity.agent_id
--  -- END QUESTION
--  
--  -- QUESTION: do we really need these two fields here? can we calculate the quality on the fly by aggregating from execution records?
--  quality_score                        DOUBLE NOT NULL,                           -- a quality score in double [0, 1]
--  quality_sample_size                  BIGINT NOT NULL,                           -- sample size from which the quality score is computed
--  -- END QUESTION
--  
--  UNIQUE INDEX (agent_name, version),                                             -- the combination of the agent's name and agent's version should be unique
--  INDEX (cardinality),
--  FOREIGN KEY collection_internal_id REFERENCES reg_agent_entity(agent_internal_id),
--  FOREIGN KEY agent_internal_id REFERENCES reg_entity(entity_internal_id)
-- ) ENGINE=InnoDB;
...
-- DROP TABLE IF EXISTS reg_process_entity;
-- CREATE TABLE reg_process_entity (
--  process_internal_id                  BIGINT NOT NULL,                           -- foreign key to reg_entity.entity_internal_id
--  process_name                         VARCHAR(511) NOT NULL,                     -- a name of the process, e.g. URI
--  version                              VARCHAR(31) NULL,                          -- optional version of the process
--  creation_time                        DATETIME NOT NULL                          -- creation of the process
--  
--  granularity                          VARCHAR(15) NOT NULL,                      -- PROCESS or METHOD
--  host_process_internal_id             BIGINT NULL,                               -- foreign key to reg_process_entity.process_internal_id
--  
--  
--  UNIQUE INDEX (process_name, version),
--  INDEX (granularity),
--  FOREIGN KEY process_internal_id REFERENCES reg_entity(entity_internal_id),
--  FOREIGN KEY host_process_internal_id REFERENCES reg_process_entity(process_internal_id)
-- ) ENGINE=InnoDB;
...
CREATE TABLE reg_entity_annotation (
  ...
  annotation_value              VARCHAR(511) NOT NULL,                            -- value part of the name-value pair.
...
CREATE TABLE exe_entity_annotation (
  ...
  annotation_value              VARCHAR(511) NOT NULL,
